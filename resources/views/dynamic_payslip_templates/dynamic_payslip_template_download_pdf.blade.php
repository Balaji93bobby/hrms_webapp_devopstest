...
$client_logo_path = public_path($data['client_details'][0]['client_logo']);

$client_image = base64_encode(file_get_contents($client_logo_path));

$abs_public_logo = public_path($data['date_month']['abs_logo']);

$abs_logo = base64_encode(file_get_contents($abs_public_logo));

$date = date_create($data['personal_details'][0]['doj']);
    ...
    <table style="width: 100%;  border-collapse: collapse; background:#fff;  ">
        <tr>
            <td colspan="3" style="margin-top:-10px">
                <h3 style="color:#000; ">PAYSLIP <span
                        style="color:#6b7280; font-family: 'Poppins', sans-serif !important;">{{ $data['date_month']['Month'] }}
                        {{ $data['date_month']['Year'] }}</span></h3>

                <h1 style="color:#000;font-size:10px; margin-top:-10px;">{{ $data['client_details'][0]['client_fullname'] }}
                </h1>
                <div style=" text-align: justify; width:200px;">
                    <p style="color:#000; font-size:9px;  text-align:left; margin-top:-10px;">

                        {{ $data['client_details'][0]['address'] }}</p>
                </div>
        ...
        <tr class="td" style="height:10px; marign:0px;">
            <td colspan="4" class="td" border="2" style="marign:0px;">
                <h5 style="font-size: 12px; margin-bottom:2px ;margin-top:-4px;">Employee Name :
                    {{ $data['personal_details'][0]['name'] }}</h5>
            </td>
        </tr>
        <tr>
            <td colspan="4">
        ...
        <tr class="td" style="height: 30px">
            <td class="" style="height: 30px;width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['user_code'] ?? '-' }}</p>
            </td>
            <td style="width:25%;">
            ...
            <td style="width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['department_name'] ?? '-' }}</p>
            </td>
            <td class="" style="height: 30px;width:25%;">
                <p style="font-size:10px;color:#383838; margin-top:-2px">Designation </p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['designation'] ?? '-' }}</p>
            </td>
        ...
        <tr class="td" style="height: 30px">
            ...
            <td style="width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">
                    {{ $data['personal_details'][0]['bank_account_number'] ? 'Bank' : 'Cheque' }}</p>
            </td>

            <td style="width:25%;">
                <p style="font-size:10px;color:#383838; margin-top:-4px">Bank Name</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['bank_name'] ?? '-' }}</p>
            </td>
            <td class="" style="height: 30px;width:25%;">
                <p style="font-size:10px;color:#383838;margin-top:-4px">Bank Account</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['bank_account_number'] ?? '-' }}</p>
            </td>
            <td style="width:25%;">
                <p style="font-size:10px;color:#383838;margin-top:-4px">Bank IFSC</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['bank_ifsc_code'] ?? '-' }}</p>
            </td>
        ...
        <tr class="td" style="height: 30px">
            ...
            <td style="width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['pan_number'] ?? '-' }}</p>
            </td>
            <td style="width:25%;">
            ...
            <td style="width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['uan_number'] ?? '-' }}</p>
            </td>
            <td style="width:25%;">
                <p style="font-size:10px;color:#383838;margin-top:-4px">EPF Number</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['personal_details'][0]['epf_number'] ?? '-' }}</p>
            </td>
        ...
        @if (!empty($data['leave_data']))
            ...
            <tr class="td" style="height: 20px">
                ...
                <td class="" style="height: 20px;width:25%;">
                    ...
                    @for ($i = 0; $i < count($data['leave_data']); $i++)
                        <p style="font-size:11px; margin-top:-8px">{{ $data['leave_data'][$i]['leave_type'] }}</p>
                    @endfor
                </td>
                <td style="width:25%;">
                    <p style="font-size:10px;color:#383838;">Opening Balance</p>
                    @for ($i = 0; $i < count($data['leave_data']); $i++)
                        <p style="font-size:11px; margin-top:-8px">{{ $data['leave_data'][$i]['opening_balance'] }}</p>
                    @endfor
                </td>
                <td style="width:25%;">
                    <p style="font-size:10px;color:#383838;">Availed</p>
                    @for ($i = 0; $i < count($data['leave_data']); $i++)
                        <p style="font-size:11px; margin-top:-8px">{{ $data['leave_data'][$i]['availed'] }}</p>
                    @endfor
                </td>
                <td style="width:25%;">
                    <p style="font-size:10px;color:#383838;">Closing Balance</p>
                    @for ($i = 0; $i < count($data['leave_data']); $i++)
                        <p style="font-size:11px; margin-top:-8px">{{ $data['leave_data'][$i]['closing_balance'] }}</p>
                    @endfor
                </td>
        ...
        <tr class="td" style="height: 20px">
            <td class="" style="height: 20px;width:25%;">
                ...
                <p style="font-size:11px; margin-top:-8px">{{ $data['salary_details'][0]['month_days'] }}</p>
            </td>
            <td style="width:25%;">
                <p style="font-size:10px;color:#383838;">TOTAL WORKING DAYS</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['salary_details'][0]['worked_Days'] }}</p>
            </td>
            <td style="width:25%;">
                <p style="font-size:10px;color:#383838;">LOSS OF PAY DAYS</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['salary_details'][0]['lop'] }}</p>
            </td>
            <td style="width:25%;">
                <p style="font-size:10px;color:#383838;">DAYS PAYABLE</p>
                <p style="font-size:11px; margin-top:-8px">{{ $data['salary_details'][0]['arrears_Days'] }}</p>
            </td>
        ...
        <tr style="width: 100%;">
            @if (count($data['earnings'][0]) <= 4)
                <td colspan="2" style=" marign-bottom:auto;position:relative; " class="h-200">
                    <table
                        ...
                        @foreach ($data['earnings'][0] as $earned_key => $single_earnings)
                                ...
                                <tr>
                                    ...
                                    <td style="width:20%">
                                        @if ($earned_key == 'Basic' || $earned_key == 'HRA')
                                            @foreach ($data['compensatory_data'] as $key => $single_value)
                                                <h1
                                    ...
                                    <td style="width:20%">
                                        @if (!empty($data['arrears'][0]))
                                            @foreach ($data['arrears'][0] as $key => $single_value)
                                                @if ($key == $earned_key)
                                                    <p
            ...
            @elseif(count($data['earnings'][0]) > 4)
            ...
            {{--  --}}
            @if (count($data['Tax_Deduction'][0]) <= 3)
                <td colspan="2" style=" marign-bottom:auto; position:relative; " class="h-200">
                    <table
                        ...
                        @foreach ($data['contribution'][0] as $key => $single_contribution)
                        ...
                        @foreach ($data['Tax_Deduction'][0] as $key => $single_taxdeduction)
            ...
            @elseif(count($data['Tax_Deduction'][0]) > 3)
        ...
        @foreach ($data['over_all'][0] as $key => $total_sumvalue)
