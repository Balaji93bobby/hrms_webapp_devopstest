<template>
    ...
    <div>
        <div v-if="service.current_user_role==1" class=" flex justify-center" >
        <img src="../assests/selfDashboard.png" alt="" class="w-[100%]" >
        </div>
        <div v-else class="w-full">
            <div class=" flex justify-between items-center ">
                <h1 class=" text-[#000] font-['poppins']  text-[24px] my-2"> <span class=" text-[gray]  text-[24px] "> PMS
                        Dashboard,</span> {{ service.current_user_name ? service.current_user_name : '' }} !</h1>
                <Dropdown v-model="useStore.assignment_setting_id" :options="useStore.selectedyear"
                    @change="DashboardStore.selecteddropdownyear(useStore.assignment_setting_id, 'Self')"
            ...
            <div class="grid grid-cols-4 gap-4  h-[128px]  mb-[3rem]">
                ...
                <div class="bg-[#FFEFE2] rounded-lg p-3">
                    ...
                    <div class="mt-2">
                        <span class="text-[20px] font-['poppins'] my-2">{{
                            DashboardStore.Dashboard_selfReviews_score_details }}%</span>
                        <div>
                            <ProgressBar class="progressbar_val2"
                                :value="DashboardStore.Dashboard_selfReviews_score_details"></ProgressBar>
                ...
                <div class="bg-[#F6F3D4] rounded-lg p-3">
                    ...
                    <div class="mt-2">
                        <span class="text-[20px] font-['poppins'] my-2">{{ DashboardStore.pmsDashboardReviewerScore_details
                        }}%</span>
                        <div>
                            <ProgressBar class="progressbar_val2" :value="DashboardStore.pmsDashboardReviewerScore_details">
                            </ProgressBar>
                            <div class="flex justify-between my-2 font-['poppins']"><span>Score</span></div>
                        </div>
                    </div>
...
onMounted(async () => {
    ...
    await DashboardStore.PerformanceRatingGraph(user_code, 'Self');
    await DashboardStore.pmsDashboardSelfReviewsScore(user_code, 'Self');
    await DashboardStore.pmsDashboardReviewerScore(user_code, 'Self');
    await DashboardStore.pmsSelfReviewOnTime(user_code);
    await DashboardStore.pmsReviewerReviewOnTime(user_code);
    await DashboardStore.pmsDashboardQuaterlyScoreCard(user_code, 'Self');
    await DashboardStore.getPmsTrendListOptions(user_code, 'Self');
    await DashboardStore.getPmsTrendList(user_code, null);
    await DashboardStore.Dashboardprocess(user_code, 'Self');
});
