...
<span></span>
    ...
    <Dropdown v-model="useStore.assignment_setting_id" :options="useStore.selectedyear"  @change="DashboardStore.selecteddropdownyear(useStore.assignment_setting_id,'Team')" optionValue="assignment_id" optionLabel="assignment_period" placeholder="    Select  Year" class=" h-[30px] w-[220px]  " />
</div>
...
await DashboardStore.PerformanceRatingGraph(user_code,'Team');
await DashboardStore.pmsDashboardSelfReviewsScore(user_code,'Team');
await DashboardStore.pmsDashboardReviewerScore(user_code,'Team');
await DashboardStore.pmsSelfReviewOnTime(user_code);
await DashboardStore.pmsReviewerReviewOnTime(user_code);
await DashboardStore.pmsDashboardQuaterlyScoreCard(user_code,'Team');
await DashboardStore.getPmsTrendListOptions(user_code,'Team');
await DashboardStore.getPmsTrendList(user_code,null);
await DashboardStore.Dashboardprocess(user_code,'Team');
});
